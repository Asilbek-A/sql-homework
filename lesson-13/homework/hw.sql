...
/* Task 1
Savol (English):
Write a query to display the result "100-Steven King". It means to combine employee_id, first_name, and last_name in one string with the given format.
Tables Used: Employees

Savol (O‘zbekcha):
Shunday query yozingki, natijada "100-Steven King" chiqsin. Bu degani employee_id, first_name, va last_name ni birlashtirib ko‘rsatish kerak.
*/
--	Task 1
SELECT 
    CAST(EMPLOYEE_ID AS VARCHAR) + '-' + FIRST_NAME + ' ' + LAST_NAME AS FullInfo
FROM Employees
...
SELECT 
    Year1,
    (SELECT MAX(v) 
    FROM (VALUES (Max1), (Max2), (Max3)) AS ValueTable(v)) AS HighestValue
FROM TestMax;
...
SELECT 
    ...
    PARSENAME(REPLACE(Name, ' ', '.'), 2) AS MiddleName,
...
--	 Task 6
SELECT TOP 1 SpacemanID, JobDescription, DATEDIFF(YEAR, HireDate, GETDATE()) AS Experience
FROM Personal
