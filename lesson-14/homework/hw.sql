...
SELECT 
    LEFT(Name, CHARINDEX(',', Name) - 1) AS FirstName,   -- Ism
    RIGHT(Name, LEN(Name) - CHARINDEX(',', Name)) AS Surname -- Familiya
...
WITH CTE AS (
    SELECT value AS ch
    FROM STRING_SPLIT('rtcfvty34redt', '') -- SQL Server 2022+ da ishlaydi
)
SELECT 
...
SELECT 
    ...
    CASE 
        ...
        WHEN DATEDIFF(YEAR, HIRE_DATE, GETDATE()) BETWEEN 1 AND 3 THEN 'Junior'
        WHEN DATEDIFF(YEAR, HIRE_DATE, GETDATE()) BETWEEN 4 AND 6 THEN 'Mid'
        ELSE 'Senior'
